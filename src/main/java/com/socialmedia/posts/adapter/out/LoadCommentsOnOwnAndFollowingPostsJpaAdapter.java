...
public class LoadCommentsOnOwnAndFollowingPostsJpaAdapter implements LoadCommentsOnOwnAndFollowingPostsPort {
    @Override
        ...
        return JpaDatabaseUtils.doInTransactionAndReturn(entityManager -> {
            return entityManager.createQuery(
                    "SELECT new com.socialmedia.posts.adapter.in.vms.CommentReturnVM(c.postId, c.commentUserEmail, c.commentBody, c.commentCreationTime)" +
                            " FROM CommentOnOwnAndFollowingPostsViewJpa c WHERE c.userId = :userId ORDER BY c.commentCreationTime DESC",
